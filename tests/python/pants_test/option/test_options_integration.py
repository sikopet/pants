...
class TestOptionsIntegration(PantsRunIntegrationTest):
  ...
  def test_options_scope(self):
    ...
    self.assertIn('options.option = None', pants_run.stdout_data)
    self.assertNotIn('gen.wire.wire_compiler = ', pants_run.stdout_data)

    pants_run = self.run_pants(['options', '--no-colors', '--scope=gen.wire'])
    self.assert_success(pants_run)
    self.assertNotIn('options.colors = False', pants_run.stdout_data)
    self.assertNotIn('options.scope = options', pants_run.stdout_data)
    self.assertNotIn('options.option = None', pants_run.stdout_data)
    self.assertIn('gen.wire.wire_compiler = ', pants_run.stdout_data)

  def test_options_option(self):
    pants_run = self.run_pants(['options', '--no-colors', '--option=colors'])
    self.assert_success(pants_run)
  ...
  def test_options_show_overridden(self):
    pants_run = self.run_pants(['options', '--no-colors', '--only-overridden', '--show-overridden'])
    self.assert_success(pants_run)
    self.assertIn('options.only_overridden = True', pants_run.stdout_data)
