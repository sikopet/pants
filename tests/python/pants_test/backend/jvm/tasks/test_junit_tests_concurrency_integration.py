...
class JunitTestsConcurrencyIntegrationTest(PantsRunIntegrationTest):
  ...
  def test_parallel_both(self):
    ...
    with self.temporary_workdir() as workdir:
      pants_run = self.run_pants_with_workdir([
        ...
        'testprojects/tests/java/org/pantsbuild/testproject/parallelboth'
      ], workdir)
      self.assert_success(pants_run)
  ...
  def test_parallel_both_cmdline(self):
    """Checks the --test-junit-default_concurrency=PARALLEL_BOTH setting."""
    with self.temporary_workdir() as workdir:
      pants_run = self.run_pants_with_workdir([
         'test',
        '--test-junit-default-concurrency=PARALLEL_BOTH',
        '--test-junit-parallel-threads=4',
        'testprojects/tests/java/org/pantsbuild/testproject/parallelboth:cmdline'
        ], workdir)
      self.assert_success(pants_run)
      self.assertIn("OK (4 tests)", pants_run.stdout_data)
