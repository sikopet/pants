...
class JvmPlatformExplainTest(JvmPlatformAnalysisTestMixin, TaskTestBase):
  ...
  def test_change_only_quiet(self):
    ...
    self.assertTrue('Allowable JVM platform ranges' in lines[0])
  ...
  def test_upgradeable(self):
    ...
    self.assertTrue('java:one' not in text, text)
    self.assertTrue('java:three' in text, text)
    self.assertTrue('java:two' in text, text)
  ...
  def test_downgradeable(self):
    ...
    text = '\n'.join(self.get_lines([one, two, nope], colors=False, ranges=False,
                                    downgradeable=True))
    self.assertTrue('java:one' in text, text)
    self.assertTrue('java:nope' not in text, text)
    self.assertTrue('java:two' in text, text)
