...
class JvmTarget(Target, Jarable):
  ...
  def traversable_dependency_specs(self):
    ...
    for resource_spec in self._resource_specs:
      yield resource_spec
    # TODO(Eric Ayers): should this be here or in target.py to catch non jvm targets?
    #  Return any SourceSets declared in payload fields
    for payload_field in self.payload:
      field_value = self.payload.get_field_value(payload_field)
      if isinstance(field_value, SourcesField):
        if not field_value.source_set.is_synthetic:
          yield field_value.source_set.address.spec
