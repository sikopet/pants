...
class IvyImportsFingerprintStrategy(DefaultFingerprintHashingMixin, FingerprintStrategy):

  def compute_fingerprint(self, target):
    if isinstance(target, JarLibrary):
      return target.payload.fingerprint()
    if isinstance(target, ImportJarsMixin):
      # This is too early to get the full fingerprint, just get the string specs that the
      # target is going to import.
      hasher = sha1()
      for jar_import in sorted(target.imported_jar_library_specs):
        hasher.update(jar_import)
...
class IvyImports(IvyTaskMixin, NailgunTask):
  ...
  def execute(self):
    ...
    fingerprint_strategy = IvyImportsFingerprintStrategy()
    ...
    with self.invalidated(all_targets,
