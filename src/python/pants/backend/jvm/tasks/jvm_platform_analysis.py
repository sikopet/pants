...
class JvmPlatformAnalysisMixin(object):
  ...
  def jvm_targets(self):
    return self.context.targets(self.is_jvm_target)
...
class JvmPlatformValidate(JvmPlatformAnalysisMixin, TaskBase):
...
class JvmPlatformExplain(JvmPlatformAnalysisMixin, ConsoleTask):
  ...
  def __init__(self, *args, **kwargs):
    ...
    self.explain_regex = self.get_options().filter
    self.detailed = self.get_options().detailed
    self.only_broken = self.get_options().only_broken
    if self.explain_regex:
      self.explain_regex = re.compile(self.explain_regex)

    self.min_allowed_version = None
    self.max_allowed_version = None
    self.target_dependencies = None
    self.target_dependees = None
  ...
  def _is_relevant(self, target):
    if not self.is_jvm_target(target):
      return False
  ...
  def _compute_ranges(self):
    ...
    for target, deps in self.transitive_dependency_map.items():
      if self.is_jvm_target(target):
        target_dependencies[target] = filter(self.is_jvm_target, deps)
  ...
  def possible_version_evaluation(self):
    ...
    for target in sorted(filter(self._is_relevant, self.jvm_targets)):
      ...
      if self.detailed or not current_valid:
        if min_version:
          min_because = [t for t in self.target_dependencies[target]
        ...
        if max_version:
          max_because = [t for t in self.target_dependees[target]
  ...
  def downgradeable(self):
    ...
    return self._changeable('downgrade',
                            can_change=lambda curr, next: curr > next,
                            change_getter=self.min_allowed_version.get)

  def upgradeable(self):
    if self.max_allowed_version is None:
      self._compute_ranges()
    return self._changeable('upgrade',
                            can_change=lambda curr, next: curr < next,
                            change_getter=self.max_allowed_version.get)
