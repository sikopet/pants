...
public class ParallelMethodsDefaultParallelTest1 {
  ...
  static void awaitLatch(String methodName) throws Exception {
    ...
    // parallel, it only insures that at least two methods get started and that no more than
    // 2 methods run at a time. A better test would show that methods are run in parallel
    // in each class.

    System.out.println("start " + methodName);
    latch.countDown();
    // Make sure that we wait for at least 2 methods to get started to insure there is some
